...
def get_monitors_count() -> int:
    ...
    try:
        with mss.mss() as sct:
            # monitors[0] is all monitors combined, [1..n] are physical displays
...
def get_monitors_info() -> list[dict]:
    """Return resolution details of all connected monitors."""
    if not _MSS:
        return []
    try:
        with mss.mss() as sct:
            ...
            for i, m in enumerate(sct.monitors):
                label = "All Displays Combined" if i == 0 else f"Monitor {i}"
                info.append({
                    "index": i,
                    "name": label,
                    "width": m["width"],
...
def _capture_screen(monitor: int | str = 1) -> tuple[bytes, str]:
    """
    Capture active screen image.
    monitor: 1 for primary, 2 for secondary, etc., or 'all'/'combined' for all monitors.
    """
    if not _MSS:
    ...
    with mss.mss() as sct:
        ...
        total_real = len(monitors) - 1

        mon_str = str(monitor).strip().lower()
        if mon_str in ("all", "0", "both", "combined"):
            target = monitors[0]
        else:
            try:
                idx = int(mon_str)
                if 1 <= idx <= total_real:
                    ...
                    target = monitors[1] if total_real >= 1 else monitors[0]
