...
from actions.screen_processor  import _capture_camera, _capture_screen, get_monitors_info
from actions.goal_agent        import execute_autonomous_goal
from actions.home_automation   import home_automation_control
from actions.email_calendar    import email_calendar_action
from actions.copilot_watcher   import copilot_control
...
class UltronLive:
    ...
    async def _handle_screen_process(self, args, loop):
        ...
        if self._vision_busy or (_now - self._vision_last_time) < _cooldown:
            ...
            if angle == "camera":
                ...
                img_b, mime_t = await loop.run_in_executor(None, lambda: _capture_screen(monitor=monitor))
                print(f"[Vision] 🖥️  Screen (Display {monitor}): {len(img_b):,} bytes")
                _stall = f"screen (Display {monitor})"
    ...
    async def _handle_autonomous_goal(self, args, loop):
        r = await loop.run_in_executor(
            None,
            lambda: execute_autonomous_goal(parameters=args, player=self.ui, speak=self.speak)
        )
        return r or "Goal completed."

    async def _handle_smart_home(self, args, loop):
        r = await loop.run_in_executor(None, lambda: home_automation_control(parameters=args, player=self.ui))
        return r or "Smart home command executed."

    async def _handle_email_calendar(self, args, loop):
        r = await loop.run_in_executor(None, lambda: email_calendar_action(parameters=args, player=self.ui))
        return r or "Email/Calendar action completed."

    async def _handle_copilot(self, args, loop):
        r = await loop.run_in_executor(
            None,
            lambda: copilot_control(parameters=args, player=self.ui, speak=self.speak)
        )
        return r or "Copilot status updated."
    ...
    TOOL_REGISTRY = {
        ...
        "execute_autonomous_goal": _handle_autonomous_goal,
        "smart_home_control": _handle_smart_home,
        "email_calendar_control": _handle_email_calendar,
        "copilot_control": _handle_copilot,
